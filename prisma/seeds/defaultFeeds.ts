...
export async function seedDefaultFeeds(prismaClient: PrismaClient = new PrismaClient()) {
...
async function main() {
  ...
  try {
    const result = await seedDefaultFeeds(prisma);
    console.log('[defaultFeeds seed] result:', result);
  } finally {
    await prisma.$disconnect();
  }
}

const isDirectRun = (process.argv[1] || '').includes('defaultFeeds.ts');

if (isDirectRun) {
  main().catch((error) => {
    console.error('[defaultFeeds seed] failed:', error);
    process.exit(1);
  });
}
